...
- **Webview script injection syntax errors**: Fixed critical bug where analysis webview failed with `SyntaxError: Invalid or unexpected token` when PR diffs or analysis content contained `</script>` sequences. Root cause: `JSON.stringify()` output was embedded directly in HTML `<script>` tags, and content containing `</script>` or other HTML-breaking sequences would prematurely terminate the script block. Fix: Using `html-safe-json` library for safe serialization in both analysis and tool testing webviews.
- **Webview module script timing**: Fixed critical bug where webview failed on first open but worked on second open. Root cause: Module scripts (`<script type="module">`) have implicit `defer`, meaning they execute **after** `DOMContentLoaded` has already fired. The `DOMContentLoaded` event listener was added too late and never triggered. Fix: Created `onDomReady()` utility that checks `document.readyState` and either waits for the event or executes immediately if DOM is already ready.
