...
- **Self-reflection scoring** (tool-based 2nd pass): After the main analysis, all surviving findings are presented back to the model with their evidence and relevant diff context. The model calls `score_finding` tool for each finding, assigning a 1-10 confidence score via structured tool args (no raw text parsing). Findings below a per-model threshold are dropped as likely false positives. Confidence scores are displayed to the user in a collapsible table. Per-model thresholds: dismissive models (GPT-4.1/4o) = 5, balanced (Claude, Raptor) = 7, aggressive (GPT-5-mini) = 8. Expected 30-50% reduction in false positives.
- **`list_directory` tool removed**: Globally removed from tool registry — `find_symbol` and `search_for_pattern` are strictly better for code review. Reduces cognitive load for all models, especially GPT-4.1 which benefits from fewer tool choices.
