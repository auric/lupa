...
- **Self-reflection scoring** (Qodo-style 2nd pass): After the main analysis, all surviving findings are presented back to the model with their evidence and relevant diff context. The model assigns a 1-10 confidence score to each finding. Findings below a per-model threshold are dropped as likely false positives. Replaces the simpler binary self-critique pass. Per-model thresholds: dismissive models (GPT-4.1/4o) = 5, balanced (Claude, Raptor) = 7, aggressive (GPT-5-mini) = 8. Expected 30-50% reduction in false positives.
