...
vscodeMock.Position = vi.fn(function (line, character) {
    ...
    this.isEqual = vi.fn(function (other) {
        return other && other.line === line && other.character === character;
    });
    this.isBefore = vi.fn(function (other) {
        return (
            other &&
            (line < other.line ||
    ...
    this.isAfter = vi.fn(function (other) {
        return (
            other &&
            (line > other.line ||
                (line === other.line && character > other.character))
...
vscodeMock.Range = vi
    ...
    .mockImplementation(
        function (
            ...
            let startPos, endPos;
            if (
                typeof startOrStartLine === 'number' &&
                typeof endOrStartCharacter === 'number'
            ) {
                startPos = new vscodeMock.Position(
                    startOrStartLine,
                    endOrStartCharacter
                );
            ...
            if (
                !startPos ||
                typeof startPos.line !== 'number' ||
                typeof startPos.character !== 'number'
            ) {
                console.error(
                    '[VSCODE_MOCK Range] Invalid start position:',
                    startPos
                );
            ...
            if (
                !endPos ||
                typeof endPos.line !== 'number' ||
                typeof endPos.character !== 'number'
            ) {
                console.error(
                    '[VSCODE_MOCK Range] Invalid end position:',
                    endPos
                );
            ...
            this.contains = vi.fn(function (positionOrRange) {
                ...
                return (
                    !positionOrRange.start.isBefore(startPos) &&
                    !positionOrRange.end.isAfter(endPos)
                );
            });
            this.isEqual = vi.fn(function (other) {
                return (
                    other &&
                    startPos.isEqual(other.start) &&
                    endPos.isEqual(other.end)
                );
            });
...
vscodeMock.Location = vi
    .fn()
    .mockImplementation(function (uri, rangeOrPosition) {
        this.uri = uri;
        this.range = rangeOrPosition;
    });

// Add custom mocks for InlineCompletionItem class - Vitest 4 requires function syntax
vscodeMock.InlineCompletionItem = vi
    .fn()
    .mockImplementation(function (insertText, range, command) {
...
vscodeMock.FileStat = vi
    .fn()
    .mockImplementation(function (type, ctime, mtime, size) {
...
vscodeMock.Uri = vi.fn(function (scheme, authority, path, query, fragment) {
    ...
    this.toString = vi.fn((_skipEncoding = false) => {
...
vscodeMock.Uri.parse = vi.fn((value, _strict = false) => {
    ...
    try {
        ...
        console.warn(
            `[VSCODE_MOCK Uri.parse] Failed to parse "${value}" with URL constructor, using basic file path assumption. Error: ${e.message}`
        );
...
vscodeMock.Uri.joinPath = vi.fn((baseUri, ...pathSegments) => {
    if (!(baseUri instanceof vscodeMock.Uri)) {
        ...
        if (
            baseUri &&
            typeof baseUri.fsPath === 'string' &&
            typeof baseUri.scheme === 'string'
        ) {
            baseUri = new vscodeMock.Uri(
                baseUri.scheme,
                baseUri.authority || '',
                baseUri.path || '',
                baseUri.query || '',
            ...
            if (
                baseUri.scheme === 'file' &&
                typeof arguments[0].fsPath === 'string'
            ) {
            ...
            console.error(
                '[VSCODE_MOCK Uri.joinPath] baseUri is not an instance of vscodeMock.Uri:',
                baseUri
            );
    ...
    const joinedUri = new vscodeMock.Uri(
        baseUri.scheme,
        baseUri.authority,
        newPath,
        baseUri.query,
...
vscodeMock.LanguageModelChatMessage = vi
    .fn()
    .mockImplementation(function (role, content, name) {
        this.role = role;
...
vscodeMock.LanguageModelChat = vi
    .fn()
    .mockImplementation(
        function (id, name, vendor, family, version, maxInputTokens) {
...
export const LanguageModelToolResultPart =
    vscodeMock.LanguageModelToolResultPart;
...
export const ChatResponseCommandButtonPart =
    vscodeMock.ChatResponseCommandButtonPart;
