...
export class SymbolFormatter {
    ...
    static convertKindStringToNumber(kindString: string): number | undefined {
        ...
        return allKinds.find(
            (kind) => this.getSymbolKindName(kind) === targetString
        );
    }
    ...
    static formatSymbolsWithHierarchy(
        ...
        const processSymbols = (
            ...
            for (const symbol of symbolList) {
                ...
                if (options.excludeKinds?.includes(symbol.kind)) {
                    continue;
                }
                if (
                    options.includeKinds &&
                    !options.includeKinds.includes(symbol.kind)
                ) {
                    continue;
                }
                ...
                const indent = options.showHierarchy
                    ? '  '.repeat(currentDepth)
                    : '';
                ...
                if (options.includeBody && document && symbol.range) {
                    try {
                        ...
                        const bodyIndent = options.showHierarchy
                        ...
                        for (
                            ...
                            if (line && line.trim()) {
                                ...
                                lines.push(
                                    `${bodyLineNumber}: ${bodyIndent}${line.trim()}`
                                );
                            }
                        }
                        ...
                        if (bodyLines.length > 5) {
                            lines.push(
                                `${lineNumber + 5}: ${bodyIndent}... (truncated)`
                            );
                        }
                    } catch {
                ...
                if (
                    ...
                    symbolCount < options.maxSymbols
                ) {
