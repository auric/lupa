...
export class SymbolRangeExpander {
   ...
   */
  async getFullSymbolRange(document: vscode.TextDocument, symbolRange: vscode.Range): Promise<vscode.Range> {
    try {
      ...
      if (symbols && symbols.length > 0) {
        ...
        const containingSymbol = this.findContainingSymbol(symbols, targetPosition);
   ...
   */
  private findContainingSymbol(symbols: vscode.DocumentSymbol[], targetPosition: vscode.Position): vscode.DocumentSymbol | undefined {
    for (const symbol of symbols) {
      if (symbol.range.contains(targetPosition)) {
        // Check if any child symbol is more specific
        if (symbol.children && symbol.children.length > 0) {
          const childSymbol = this.findContainingSymbol(symbol.children, targetPosition);
          if (childSymbol) {
            return childSymbol;
          }
        }
   ...
   */
  private expandRangeForSymbol(document: vscode.TextDocument, symbolRange: vscode.Range): vscode.Range {
