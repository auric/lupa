...
export class ReadFileTool extends BaseTool {
  ...
  schema = z.object({
    ...
    start_line: z.number().min(1).optional()
      .describe('Starting line number (1-based). If provided alone, reads from this line to end of file (up to max lines limit)'),
    end_line: z.number().min(1).optional()
      .describe('Ending line number (1-based, inclusive). Cannot be used together with line_count'),
    line_count: z.number().min(1).optional()
      .describe('Number of lines to read from start_line. Cannot be used together with end_line')
  }).refine(
    (data) => !(data.end_line !== undefined && data.line_count !== undefined),
    { message: 'Cannot specify both end_line and line_count. Use one or the other.' }
  );
  ...
  private calculateReadRange(params: {
    ...
    totalLines: number;
  }): { success: true; actualStartLine: number; actualEndLine: number; wasTruncated: boolean } | { success: false; error: string } {
    ...
    if (startLine === undefined && endLine === undefined && lineCount === undefined) {
      if (totalLines <= maxLines) {
        return { success: true, actualStartLine: 1, actualEndLine: totalLines, wasTruncated: false };
      }
      return {
        success: true,
        actualStartLine: 1,
    ...
    if (actualStartLine > totalLines) {
      return { success: false, error: `Start line ${actualStartLine} exceeds file length (${totalLines} lines)` };
    }

    // Case 2: end_line specified
    if (endLine !== undefined) {
      if (endLine < actualStartLine) {
        return { success: false, error: `end_line (${endLine}) must be >= start_line (${actualStartLine})` };
      }
      ...
      if (requestedLines > maxLines) {
        return {
          success: false,
          error: `Requested ${requestedLines} lines exceeds maximum of ${maxLines}. ` +
      ...
      return { success: true, actualStartLine, actualEndLine: clampedEndLine, wasTruncated: false };
    }
    ...
    if (lineCount !== undefined) {
      if (lineCount > maxLines) {
        return {
          success: false,
          error: `Requested ${lineCount} lines exceeds maximum of ${maxLines}. ` +
    ...
    if (remainingLines <= maxLines) {
      return { success: true, actualStartLine, actualEndLine: totalLines, wasTruncated: false };
    }
