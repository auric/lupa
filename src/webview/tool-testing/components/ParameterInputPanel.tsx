...
export const ParameterInputPanel: React.FC<ParameterInputPanelProps> = ({
    ...
    const [parameters, setParameters] =
        useState<Record<string, any>>(initialParameters);
    ...
    const handleExecute = async () => {
        if (!toolInfo || isExecuting) {
            return;
        }

        const filteredParams = Object.entries(parameters).reduce(
            (acc, [key, value]) => {
    ...
    const handleClearForm = () => {
        const clearedParams = parameterInfos.reduce(
            (acc, param) => {
...
const ParameterField: React.FC<ParameterFieldProps> = ({
    ...
    const renderInput = () => {
                ...
                <div className="flex items-center space-x-2">
                    <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                            onChange(parameter.name, checked)
                        }
                        aria-describedby={
                            parameter.description
                                ? `${parameter.name}-desc`
        ...
        const isLargeText =
            parameter.name.toLowerCase().includes('content') ||
