...
export const useToolExecution = () => {
    ...
    const [currentSession, setCurrentSession] =
        useState<ToolTestSession | null>(null);
    ...
    const executeToolTest = async (
        ...
        try {
            ...
            const executionPromise = new Promise<ToolTestResult[]>(
                (resolve, reject) => {
                    const messageHandler = (event: MessageEvent) => {
                        const message = event.data;

                        if (
                            message.type === 'toolExecutionResult' &&
                            ...
                            if (message.payload.error) {
                                ...
                                const results: ToolTestResult[] =
                                    message.payload.results.map(
                                        (data: any, index: number) => ({
                                            id: `result-${index}`,
                                            data,
                                            timestamp: new Date(),
                                            executionTime:
                                                message.payload.executionTime ||
                                                0,
                            ...
                            message.type === 'toolExecutionError' &&
                            message.payload.sessionId === session.id
                        ) {
                            window.removeEventListener(
                                'message',
