...
describe('ToolCallingAnalysisProvider Enhanced Integration', () => {
  ...
  describe('Token validation and context management', () => {
    it('should handle large conversations by cleaning up old context', async () => {
      ...
      const longHistory = [
        ...
        { role: 'tool', content: 'A'.repeat(2000), toolCalls: undefined, toolCallId: 'call_1' },
        { role: 'assistant', content: 'I will call another tool', toolCalls: [{ id: 'call_2', function: { name: 'tool2', arguments: '{}' } }], toolCallId: undefined },
        { role: 'tool', content: 'B'.repeat(2000), toolCalls: undefined, toolCallId: 'call_2' }
      ];
  ...
  describe('Tool response size validation', () => {
    it('should handle tools that return responses exceeding size limits', async () => {
      ...
      const mockReadFileTool = {
        ...
        schema: {},
        getVSCodeTool: () => ({ name: 'read_file', description: 'Read file', inputSchema: {} })
      };

      mockToolExecutor.getAvailableTools.mockReturnValue([mockReadFileTool]);
      ...
      mockCopilotModelManager.sendRequest
        .mockResolvedValueOnce({
          ...
          toolCalls: [{
            id: 'call_1',
            function: { name: 'read_file', arguments: JSON.stringify({ filePath: 'large-file.ts' }) }
          }]
        })
        .mockResolvedValueOnce({
  ...
  describe('Diff size processing and tool availability', () => {
    it('should disable tools when diff is too large', async () => {
      ...
      expect(mockConversationManager.addUserMessage).toHaveBeenCalledWith(
        expect.stringContaining(TokenConstants.TOOL_CONTEXT_MESSAGES.TOOLS_DISABLED)
      );
    });

    it('should keep tools available when diff is reasonably sized', async () => {
      const reasonableDiff = 'diff --git a/file.ts b/file.ts\nindex abc..def\n--- a/file.ts\n+++ b/file.ts\n@@ -1,3 +1,3 @@\n-old line\n+new line';
      ...
      expect(mockConversationManager.addUserMessage).toHaveBeenCalledWith(
        expect.not.stringContaining(TokenConstants.TOOL_CONTEXT_MESSAGES.TOOLS_DISABLED)
      );
    });
  });
  ...
  describe('Complete workflow integration', () => {
    it('should handle full analysis workflow with tool calls and context management', async () => {
      ...
      mockCopilotModelManager.sendRequest
        .mockResolvedValueOnce({
          content: 'I need to read the file to understand the changes',
          toolCalls: [{
            id: 'call_1',
            function: { name: 'read_file', arguments: JSON.stringify({ filePath: 'src/test.ts' }) }
          }]
        })
        .mockResolvedValueOnce({
          content: 'Based on the file content, here is my analysis: The function was renamed from old() to new().',
          toolCalls: undefined
        });
      ...
      expect(mockConversationManager.addAssistantMessage).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([expect.objectContaining({ id: 'call_1' })])
      );
      expect(mockConversationManager.addToolMessage).toHaveBeenCalledWith(
        'call_1',
