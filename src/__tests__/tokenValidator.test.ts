...
describe('TokenValidator', () => {
  ...
  describe('validateTokens', () => {
    it('should return continue action when tokens are within safe limits', async () => {
      ...
      const result = await tokenValidator.validateTokens(messages, systemPrompt);
    ...
    it('should return remove_old_context action when exceeding warning threshold', async () => {
      const systemPrompt = 'You are a helpful assistant';
      const messages: ToolCallMessage[] = [
        { role: 'user', content: 'Long message...', toolCalls: undefined, toolCallId: undefined }
      ];
    ...
    it('should return request_final_answer action when exceeding max tokens', async () => {
      const systemPrompt = 'You are a helpful assistant';
      const messages: ToolCallMessage[] = [
        { role: 'user', content: 'Very long message...', toolCalls: undefined, toolCallId: undefined }
      ];
    ...
    it('should handle tool calls in token counting', async () => {
      const systemPrompt = 'You are a helpful assistant';
      const toolCalls = [
        { id: 'call_1', function: { name: 'test_tool', arguments: '{"param": "value"}' } }
      ];
      const messages: ToolCallMessage[] = [
        { role: 'assistant', content: 'I will use a tool', toolCalls, toolCallId: undefined }
      ];
  ...
  describe('cleanupContext', () => {
    it('should remove oldest tool interactions when over target', async () => {
      ...
      const messages: ToolCallMessage[] = [
        ...
        { role: 'assistant', content: 'I will call a tool', toolCalls: [{ id: 'call_1', function: { name: 'tool1', arguments: '{}' } }], toolCallId: undefined },
        { role: 'tool', content: 'Tool result 1', toolCalls: undefined, toolCallId: 'call_1' },
        { role: 'assistant', content: 'I will call another tool', toolCalls: [{ id: 'call_2', function: { name: 'tool2', arguments: '{}' } }], toolCallId: undefined },
        { role: 'tool', content: 'Tool result 2', toolCalls: undefined, toolCallId: 'call_2' },
        { role: 'user', content: 'Final request', toolCalls: undefined, toolCallId: undefined }
      ];
      ...
      const result = await tokenValidator.cleanupContext(messages, systemPrompt, 0.5);
      ...
      const lastMessage = result.cleanedMessages[result.cleanedMessages.length - 1];
      expect(lastMessage.role).toBe('user');
      expect(lastMessage.content).toBe(TokenConstants.TOOL_CONTEXT_MESSAGES.CONTEXT_FULL);
    });
    ...
    it('should not modify messages when under target utilization', async () => {
      ...
      const result = await tokenValidator.cleanupContext(messages, systemPrompt, 0.8);
  ...
  describe('isResponseSizeAcceptable', () => {
    it('should return true for responses within limit', () => {
      ...
      expect(tokenValidator.isResponseSizeAcceptable(shortResponse)).toBe(true);
    });

    it('should return false for responses exceeding limit', () => {
      const longResponse = 'A'.repeat(TokenConstants.MAX_TOOL_RESPONSE_CHARS + 1);
      expect(tokenValidator.isResponseSizeAcceptable(longResponse)).toBe(false);
    });

    it('should handle responses exactly at the limit', () => {
      const exactResponse = 'A'.repeat(TokenConstants.MAX_TOOL_RESPONSE_CHARS);
      expect(tokenValidator.isResponseSizeAcceptable(exactResponse)).toBe(true);
    });
  });
  ...
  describe('removeOldestToolInteraction', () => {
    it('should correctly match tool results with assistant messages', async () => {
      const messages: ToolCallMessage[] = [
        { role: 'user', content: 'Initial request', toolCalls: undefined, toolCallId: undefined },
        { role: 'assistant', content: 'I will call tool1', toolCalls: [{ id: 'call_1', function: { name: 'tool1', arguments: '{}' } }], toolCallId: undefined },
        { role: 'tool', content: 'Tool1 result', toolCalls: undefined, toolCallId: 'call_1' },
        { role: 'assistant', content: 'I will call tool2', toolCalls: [{ id: 'call_2', function: { name: 'tool2', arguments: '{}' } }], toolCallId: undefined },
    ...
    it('should return not found when no tool results exist', async () => {
      const messages: ToolCallMessage[] = [
        { role: 'user', content: 'Hello', toolCalls: undefined, toolCallId: undefined },
        { role: 'assistant', content: 'Hello back', toolCalls: undefined, toolCallId: undefined }
      ];
