...
describe('PathSanitizer', () => {
    describe('sanitizePath', () => {
        describe('valid paths', () => {
            it('should accept simple relative paths', () => {
                ...
                expect(PathSanitizer.sanitizePath('src/components')).toBe('src/components');
                expect(PathSanitizer.sanitizePath('src/utils/helper.ts')).toBe('src/utils/helper.ts');
            });

            it('should accept current directory reference', () => {
                expect(PathSanitizer.sanitizePath('.')).toBe('.');
                expect(PathSanitizer.sanitizePath('./src')).toBe('src');
                expect(PathSanitizer.sanitizePath('./src/file.ts')).toBe('src/file.ts');
            });
            ...
            it('should normalize redundant path separators', () => {
                expect(PathSanitizer.sanitizePath('src//utils')).toBe('src/utils');
                expect(PathSanitizer.sanitizePath('src///components//Button.tsx')).toBe('src/components/Button.tsx');
            });
            ...
            it('should preserve file extensions', () => {
                ...
                expect(PathSanitizer.sanitizePath('src/file.test.tsx')).toBe('src/file.test.tsx');
            });
        });

        describe('directory traversal prevention', () => {
            it('should reject paths starting with parent directory', () => {
                expect(() => PathSanitizer.sanitizePath('..')).toThrow('Directory traversal detected');
                expect(() => PathSanitizer.sanitizePath('../etc/passwd')).toThrow('Directory traversal detected');
                expect(() => PathSanitizer.sanitizePath('..\\system32')).toThrow('Directory traversal detected');
            });

            it('should reject paths with embedded traversal', () => {
                expect(() => PathSanitizer.sanitizePath('src/../../../etc/passwd')).toThrow('Directory traversal detected');
                expect(() => PathSanitizer.sanitizePath('src/utils/../../..')).toThrow('Directory traversal detected');
            });

            it('should reject paths that normalize to traversal', () => {
                expect(() => PathSanitizer.sanitizePath('src/components/../../../secret')).toThrow('Directory traversal detected');
            });
        });

        describe('absolute path rejection', () => {
            it('should reject Unix absolute paths', () => {
                expect(() => PathSanitizer.sanitizePath('/etc/passwd')).toThrow('Absolute paths are not allowed');
                expect(() => PathSanitizer.sanitizePath('/usr/bin')).toThrow('Absolute paths are not allowed');
                expect(() => PathSanitizer.sanitizePath('/home/user/file.txt')).toThrow('Absolute paths are not allowed');
            });

            it('should reject Windows drive paths', () => {
                expect(() => PathSanitizer.sanitizePath('C:\\')).toThrow('Absolute paths are not allowed');
                expect(() => PathSanitizer.sanitizePath('C:/Windows/System32')).toThrow('Absolute paths are not allowed');
                expect(() => PathSanitizer.sanitizePath('D:\\Program Files')).toThrow('Absolute paths are not allowed');
                expect(() => PathSanitizer.sanitizePath('c:/users')).toThrow('Absolute paths are not allowed');
            });

            it('should reject UNC paths', () => {
                expect(() => PathSanitizer.sanitizePath('\\\\Server\\Share')).toThrow('Absolute paths are not allowed');
                expect(() => PathSanitizer.sanitizePath('\\\\?\\C:\\')).toThrow('Absolute paths are not allowed');
                expect(() => PathSanitizer.sanitizePath('\\\\.\\COM1')).toThrow('Absolute paths are not allowed');
            });
        });

        describe('null byte injection prevention', () => {
            it('should reject paths containing null bytes', () => {
                expect(() => PathSanitizer.sanitizePath('src\x00.txt')).toThrow('Path contains null bytes');
                expect(() => PathSanitizer.sanitizePath('\x00src/file.ts')).toThrow('Path contains null bytes');
                expect(() => PathSanitizer.sanitizePath('src/file.ts\x00')).toThrow('Path contains null bytes');
            });

            it('should reject paths with null bytes in the middle', () => {
                expect(() => PathSanitizer.sanitizePath('src/file\x00.ts')).toThrow('Path contains null bytes');
                expect(() => PathSanitizer.sanitizePath('src\x00/utils/file.ts')).toThrow('Path contains null bytes');
            });

            it('should reject paths with multiple null bytes', () => {
                expect(() => PathSanitizer.sanitizePath('src\x00\x00file.ts')).toThrow('Path contains null bytes');
            });

            it('should check null bytes before other validation', () => {
                // Null byte combined with traversal - should fail on null bytes first
                expect(() => PathSanitizer.sanitizePath('src\x00/../../../etc/passwd')).toThrow('Path contains null bytes');
                // Null byte combined with absolute path - should fail on null bytes first
                expect(() => PathSanitizer.sanitizePath('/etc\x00/passwd')).toThrow('Path contains null bytes');
            });
        });
        ...
        describe('path length limit', () => {
            ...
            it('should reject paths exceeding maximum length', () => {
                ...
                expect(() => PathSanitizer.sanitizePath(tooLongPath)).toThrow('Path exceeds maximum length of 1024 characters');
            });

            it('should count length after trimming', () => {
                const paddedPath = '   ' + 'a'.repeat(1024) + '   ';
                // After trimming, this is exactly 1024 characters
                expect(() => PathSanitizer.sanitizePath(paddedPath)).not.toThrow();
            });

            it('should provide clear error message with limit value', () => {
                const tooLongPath = 'a'.repeat(2000);
                expect(() => PathSanitizer.sanitizePath(tooLongPath)).toThrow('1024');
            });

            it('should reject long paths before checking for traversal', () => {
                const longTraversalPath = '../'.repeat(500);
                expect(() => PathSanitizer.sanitizePath(longTraversalPath)).toThrow('Path exceeds maximum length');
            });
        });
        ...
        describe('unicode normalization (NFC)', () => {
            it('should normalize decomposed characters to composed form', () => {
                ...
                expect(PathSanitizer.sanitizePath(nfdPath)).toBe(PathSanitizer.sanitizePath(nfcPath));
            });
            ...
            it('should handle paths with mixed Unicode forms', () => {
                ...
                expect(PathSanitizer.sanitizePath(path1)).toBe(PathSanitizer.sanitizePath(path2));
            });
            ...
            it('should normalize Korean hangul', () => {
                ...
                expect(PathSanitizer.sanitizePath(composedPath)).toBe(PathSanitizer.sanitizePath(decomposedPath));
            });

            it('should preserve ASCII-only paths unchanged', () => {
                const asciiPath = 'src/components/Button.tsx';
                expect(PathSanitizer.sanitizePath(asciiPath)).toBe('src/components/Button.tsx');
            });

            it('should handle common accented characters', () => {
                expect(PathSanitizer.sanitizePath('src/résumé.txt')).toBe('src/résumé.txt');
                expect(PathSanitizer.sanitizePath('src/naïve.ts')).toBe('src/naïve.ts');
            });
        });
        ...
        describe('combined security checks', () => {
            it('should check null bytes before length check', () => {
                ...
                expect(() => PathSanitizer.sanitizePath(longPathWithNull)).toThrow('null bytes');
            });
            ...
            it('should check length before Unicode normalization', () => {
                ...
                expect(() => PathSanitizer.sanitizePath(longPath)).toThrow('maximum length');
            });
    ...
    describe('isAbsolutePath', () => {
        ...
        it('should identify UNC paths', () => {
            expect(PathSanitizer.isAbsolutePath('\\\\server\\share')).toBe(true);
            expect(PathSanitizer.isAbsolutePath('\\\\?\\C:\\')).toBe(true);
