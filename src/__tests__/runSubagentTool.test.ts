...
describe('RunSubagentTool', () => {
    ...
    describe('Subagent Batching', () => {
        it('should enqueue instead of executing when batchManager is present', async () => {
            const mockExecutor = createMockExecutor();
            const tool = new RunSubagentTool(workspaceSettings);
            const { SubagentBatchManager } =
                await import('../sessions/subagentBatchManager');
            const batchManager = new SubagentBatchManager();
            const context = createSubagentExecutionContext(
                mockExecutor,
                sessionManager
            );
            context.subagentBatchManager = batchManager;
            ...
            expect(result.success).toBe(true);
            expect(result.data).toContain('queued');
            expect(result.data).toContain('1 pending');
            expect(batchManager.getPendingCount()).toBe(1);
            // Executor should NOT have been called
            expect(mockExecutor.execute).not.toHaveBeenCalled();
        });

        it('should accumulate multiple subagents in the batch queue', async () => {
            const mockExecutor = createMockExecutor();
            const tool = new RunSubagentTool(workspaceSettings);
            const { SubagentBatchManager } =
                await import('../sessions/subagentBatchManager');
            ...
            await tool.execute(
                { task: 'Review auth changes in login module' },
                context
            );
            await tool.execute(
                { task: 'Review API endpoint security validation' },
                context
            );
            ...
            // Verify queued entries have correct data
            const drained = batchManager.drain();
            expect(drained[0]!.task).toBe(
                'Review auth changes in login module'
            );
            expect(drained[1]!.task).toBe(
                'Review API endpoint security validation'
            );
        });

        it('should still validate budget before enqueuing', async () => {
            const mockExecutor = createMockExecutor();
            const tool = new RunSubagentTool(workspaceSettings);
            const { SubagentBatchManager } =
                await import('../sessions/subagentBatchManager');
            ...
            const result = await tool.execute(
                { task: 'This should fail budget check validation' },
                context
            );
        ...
        it('should execute immediately when batchManager is not present', async () => {
            const mockExecutor = createMockExecutor();
            const tool = new RunSubagentTool(workspaceSettings);
            const context = createSubagentExecutionContext(
                mockExecutor,
        ...
        it('should record spawn before enqueuing for budget tracking', async () => {
            const mockExecutor = createMockExecutor();
            const tool = new RunSubagentTool(workspaceSettings);
            const { SubagentBatchManager } =
                await import('../sessions/subagentBatchManager');
            ...
            // Spawn was recorded even though execution is deferred
            expect(sessionManager.getCount()).toBe(1);
        });
    });
});
